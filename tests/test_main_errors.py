...
def _run_main_with_args(monkeypatch: Any, capsys: Any, args: list[str]) -> _RunResult:
    ...
    # Code is usually an int, but could be None. Default to 1 for error testing.
    code = exc_info.value.code if isinstance(exc_info.value.code, int) else (0 if exc_info.value.code is None else 1)
