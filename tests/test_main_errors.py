...
def _run_main_with_args(monkeypatch: Any, capsys: Any, args: list[str]) -> _RunResult:
    ...
    parsed_code = code if isinstance(code, int) else (int(code) if isinstance(code, str) and code.isdigit() else 1)
    return _RunResult(parsed_code, captured.out, captured.err)
