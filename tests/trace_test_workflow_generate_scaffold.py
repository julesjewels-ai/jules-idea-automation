...
@pytest.mark.parametrize(
    "scaffold_data, expected_files_count, should_raise", [
        (
            # Happy Path
            {
                "requirements": ["pytest"],
                ...
                "files": [
                    ...
                    # Should be ignored by _prepare_scaffold_files
        ...
        (
            ...
            # NOTE: _prepare_scaffold_files returns early if 'files' is empty,
            # effectively ignoring 'requirements'. This test pins behavior.
            {
                "requirements": ["pytest"],
                "run_command": "make run",
                "files": []
...
def test_generate_scaffold_behavior(
    ...
    # The instructions say: "If the file uses from x import y,
    # you MUST patch src.current_module.y."
    # src/core/workflow.py imports build_readme from src.core.readme_builder
    # So we patch src.core.workflow.build_readme
    mock_build_readme = mocker.patch(
        "src.core.workflow.build_readme", return_value="# README"
    )

    workflow = IdeaWorkflow(
        github=mock_github, gemini=mock_gemini, jules=mock_jules
    )
    ...
    if should_raise and isinstance(scaffold_data, Exception):
        ...
        with pytest.raises(type(scaffold_data)):
            workflow._generate_scaffold(
                "testuser", mock_context, verbose=False
            )

        mock_gemini.generate_project_scaffold.assert_called_once_with(
            mock_context
        )
        ...
        mock_github.create_files.return_value = {
            "files_created": expected_files_count
        }
        ...
        args = mock_github.create_file.call_args[1]
        assert args['path'] == "README.md"
        assert args['content'] == "# README"
        assert args['owner'] == "testuser"
        assert args['repo'] == mock_context['slug']
        ...
        if expected_files_count > 0:
            ...
            assert (
                mock_github.create_files.call_args[1]['owner'] == "testuser"
            )
            assert (
                mock_github.create_files.call_args[1]['repo']
                == mock_context['slug']
            )
        else:
            mock_github.create_files.assert_not_called()
