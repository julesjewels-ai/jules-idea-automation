...
def _make_workflow(
    *,
    ...
    if getattr(gh.get_user, "return_value", None) is None or isinstance(gh.get_user.return_value, MagicMock):
        gh.get_user.return_value = {"login": "testuser"}
    if getattr(gh.create_repo, "return_value", None) is None or isinstance(gh.create_repo.return_value, MagicMock):
        gh.create_repo.return_value = {}
    if getattr(gh.create_file, "return_value", None) is None or isinstance(gh.create_file.return_value, MagicMock):
        gh.create_file.return_value = {}
    if getattr(gh.create_files, "return_value", None) is None or isinstance(gh.create_files.return_value, MagicMock):
        gh.create_files.return_value = {"files_created": 3}

    if getattr(gm.generate_project_scaffold, "return_value", None) is None or isinstance(
        gm.generate_project_scaffold.return_value, MagicMock
    ):
    ...
    if getattr(gm.generate_feature_maps, "return_value", None) is None or isinstance(
        gm.generate_feature_maps.return_value, MagicMock
    ):
    ...
    if getattr(jl.source_exists, "return_value", None) is None or isinstance(jl.source_exists.return_value, MagicMock):
        jl.source_exists.return_value = True
    if getattr(jl.create_session, "return_value", None) is None or isinstance(
        jl.create_session.return_value, MagicMock
    ):
