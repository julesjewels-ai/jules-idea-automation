...
def _make_workflow(
    *,
    ...
    if getattr(gh.get_user, 'return_value', None) is None or not isinstance(gh.get_user.return_value, dict):
        gh.get_user.return_value = {"login": "testuser"}
    if getattr(gh.create_repo, 'return_value', None) is None or not isinstance(gh.create_repo.return_value, dict):
        gh.create_repo.return_value = {}
    if getattr(gh.create_file, 'return_value', None) is None or not isinstance(gh.create_file.return_value, dict):
        gh.create_file.return_value = {}
    if getattr(gh.create_files, 'return_value', None) is None or not isinstance(gh.create_files.return_value, dict):
        gh.create_files.return_value = {"files_created": 3}

    if getattr(gm.generate_project_scaffold, 'return_value', None) is None or not isinstance(gm.generate_project_scaffold.return_value, dict):
    ...
    if getattr(gm.generate_feature_maps, 'return_value', None) is None or not isinstance(gm.generate_feature_maps.return_value, dict):
        gm.generate_feature_maps.return_value = {
            "mvp_features": [],
            "production_features": [],
        }

    if getattr(jl.source_exists, 'return_value', None) is None or not isinstance(jl.source_exists.return_value, bool):
        jl.source_exists.return_value = True
    if getattr(jl.create_session, 'return_value', None) is None or not isinstance(jl.create_session.return_value, dict):
