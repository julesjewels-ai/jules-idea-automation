...
def _make_workflow(
    *,
    ...
    if isinstance(gh.get_user.return_value, MagicMock) and not gh.get_user.side_effect:
    ...
    if (
        ...
        # if the caller provided a return_value, we leave it alone.
        pass

    if isinstance(gm.generate_feature_maps.return_value, MagicMock) and not gm.generate_feature_maps.side_effect:
