...
def _make_workflow(
    *,
    ...
    if gh.create_files.return_value is not None and isinstance(gh.create_files, MagicMock):
    ...
    if gm.generate_feature_maps.return_value is not None and isinstance(gm.generate_feature_maps, MagicMock):
        gm.generate_feature_maps.return_value = {
            "mvp_features": [],
            "production_features": [],
        }

    if jl.source_exists.return_value is not None and isinstance(jl.source_exists, MagicMock):
