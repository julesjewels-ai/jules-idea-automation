...
def _make_workflow(
    *,
    ...
    if gh.get_user.return_value and isinstance(gh.get_user.return_value, MagicMock):
        gh.get_user.return_value = {"login": "testuser"}
    if gh.create_repo.return_value and isinstance(gh.create_repo.return_value, MagicMock):
        gh.create_repo.return_value = {}
    if gh.create_file.return_value and isinstance(gh.create_file.return_value, MagicMock):
        gh.create_file.return_value = {}
    if gh.create_files.return_value and isinstance(gh.create_files.return_value, MagicMock):
        gh.create_files.return_value = {"files_created": 3}

    if gm.generate_project_scaffold.return_value and isinstance(gm.generate_project_scaffold.return_value, MagicMock):
    ...
    if gm.generate_feature_maps.return_value and isinstance(gm.generate_feature_maps.return_value, MagicMock):
        gm.generate_feature_maps.return_value = {
            "mvp_features": [],
            "production_features": [],
        }

    if jl.source_exists.return_value and isinstance(jl.source_exists.return_value, MagicMock):
        jl.source_exists.return_value = True
    if jl.create_session.return_value and isinstance(jl.create_session.return_value, MagicMock):
