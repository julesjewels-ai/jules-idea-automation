...
def github_client(monkeypatch: pytest.MonkeyPatch) -> GitHubClient:
    ...
    with patch("src.services.github.requests.request") as mock_request:
        ...
        mock_resp.headers.update({"x-oauth-scopes": "repo, read:org"})
...
def test_init_validates_classic_token_scope(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("src.services.github.requests.request") as mock_request:
        mock_resp = requests.Response()
        mock_resp.status_code = 200
        mock_resp.headers.update({"x-oauth-scopes": "repo, user"})
        mock_request.return_value = mock_resp
...
def test_init_allows_fine_grained_token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GITHUB_TOKEN", "fake-token")
    with patch("src.services.github.requests.request") as mock_request:
        mock_resp = requests.Response()
        mock_resp.status_code = 200
        # Fine-grained PATs don't send this header
        mock_request.return_value = mock_resp
...
def test_init_raises_on_missing_repo_scope(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GITHUB_TOKEN", "fake-token")
    with patch("src.services.github.requests.request") as mock_request:
        mock_resp = requests.Response()
        mock_resp.status_code = 200
        mock_resp.headers.update({"x-oauth-scopes": "user, read:org"})  # missing "repo"
        mock_request.return_value = mock_resp
...
def test_init_ignores_network_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GITHUB_TOKEN", "fake-token")
    with patch("src.services.github.requests.request") as mock_request:
        mock_request.side_effect = requests.exceptions.ConnectionError("Network unavailable")
