...
def github_client(monkeypatch: pytest.MonkeyPatch) -> GitHubClient:
    ...
    # Mock the __init__ scope validation network call
    with patch("src.services.github.requests.request") as mock_request:
        mock_response = requests.Response()
        mock_response.status_code = 200
        mock_response.headers = requests.structures.CaseInsensitiveDict({"x-oauth-scopes": "repo"})
...
def test_init_validates_token_scopes_success(monkeypatch: pytest.MonkeyPatch) -> None:
    """Client initializes successfully if repo scope is present."""
    monkeypatch.setenv("GITHUB_TOKEN", "fake-token")
    with patch("src.services.github.requests.request") as mock_request:
        mock_response = requests.Response()
        mock_response.status_code = 200
        mock_response.headers = requests.structures.CaseInsensitiveDict({"x-oauth-scopes": "repo, user"})
        mock_request.return_value = mock_response
...
def test_init_validates_token_scopes_missing_header(monkeypatch: pytest.MonkeyPatch) -> None:
    """Client initializes successfully for Fine-grained PATs (missing header)."""
    monkeypatch.setenv("GITHUB_TOKEN", "fake-token")
    with patch("src.services.github.requests.request") as mock_request:
        mock_response = requests.Response()
        mock_response.status_code = 200
...
def test_init_validates_token_scopes_unauthorized(monkeypatch: pytest.MonkeyPatch) -> None:
    """Client raises ConfigurationError on 401."""
...
def test_init_validates_token_scopes_missing_repo(monkeypatch: pytest.MonkeyPatch) -> None:
    """Client raises ConfigurationError if repo scope is missing."""

    monkeypatch.setenv("GITHUB_TOKEN", "fake-token")
    with patch("src.services.github.requests.request") as mock_request:
        mock_response = requests.Response()
        mock_response.status_code = 200
        mock_response.headers = requests.structures.CaseInsensitiveDict({"x-oauth-scopes": "user, gist"})
        mock_request.return_value = mock_response

        with pytest.raises(ConfigurationError, match="repo' scope is missing"):
            GitHubClient()
