...
def test_jules_client_api_error_401() -> None:
    ...
    with patch("src.services.jules.requests.request") as mock_request:
        ...
        assert "Your Jules API key seems invalid" in (excinfo.value.tip or "")
...
def test_jules_client_api_error_403() -> None:
    ...
    with patch("src.services.jules.requests.request") as mock_request:
        ...
        assert "You don't have permission" in (excinfo.value.tip or "")
...
def test_jules_client_api_error_404() -> None:
    ...
    with patch("src.services.jules.requests.request") as mock_request:
        ...
        assert "The requested resource was not found" in (excinfo.value.tip or "")
...
def test_jules_client_generic_error() -> None:
    ...
    with patch("src.services.jules.requests.request") as mock_request:
        ...
        assert "API returned status 500" in (excinfo.value.tip or "")
...
def test_jules_client_json_error() -> None:
    ...
    with patch("src.services.jules.requests.request") as mock_request:
        ...
        assert "API Message: Custom API Error" in (excinfo.value.tip or "")
