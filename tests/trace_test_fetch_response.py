...
@pytest.mark.parametrize("url, mock_setup, expected_outcome", [
    ...
    (
        ...
        ("HTTP error accessing", "The website is blocking automated access (403 Forbidden). Try a different source.")
    ),
    # Edge Case: 404 Not Found
    (
        "http://missing.com",
        {"status_code": 404, "http_error": True},
        ("HTTP error accessing", "The page was not found (404). Check the URL for typos.")
    ),
    # Error State: Timeout
    (
        "http://timeout.com",
        {"side_effect": requests.exceptions.Timeout("Read timed out")},
        ("Timeout accessing", "The website is taking too long to respond. It might be down or blocking connections.")
    ),
    # Error State: Generic RequestException
    (
        "http://error.com",
        {"side_effect": requests.exceptions.RequestException("Connection aborted")},
        ("Network error accessing", "Check your internet connection and DNS settings.")
    ),
...
def test_fetch_response_behavior(
    ...
    # We patch 'src.services.scraper.requests.get' because the module imports requests directly.
    ...
    if "side_effect" in mock_setup:
        # Exceptions that happen during the get call itself (Timeout, RequestException, etc.)
        ...
        error = requests.exceptions.HTTPError(f"{resp.status_code} Client Error")
        error.response = resp
    ...
    if isinstance(expected_outcome, tuple):
        ...
        mock_get.assert_called_once_with(url, timeout=15, headers=expected_headers)
