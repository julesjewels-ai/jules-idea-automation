...
@pytest.mark.parametrize("scaffold_fixture, feature_maps_fixture, expected", [
...
def test_target_function_behavior(
    ...
    scaffold = request.getfixturevalue(scaffold_fixture) if scaffold_fixture not in ("empty_dict", "corrupt_val") else ({} if scaffold_fixture == "empty_dict" else "corrupt_data")
    feature_maps = request.getfixturevalue(feature_maps_fixture) if feature_maps_fixture not in ("empty_dict", "none_val") else ({} if feature_maps_fixture == "empty_dict" else None)
