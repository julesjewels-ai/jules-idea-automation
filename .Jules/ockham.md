...
**Delta:** Extracted control flow to utility.
**Summary:** Extracted the polling loop and timeout management from `watch_session` into the shared `poll_with_result` utility in `src/utils/polling.py`. This separates the "what" (checking status) from the "how" (looping and sleeping), making the command handler cleaner and easier to test.
