...
Delta: Complexity Score C (19) -> A (2)
Summary: Extracted looping logic and field access abstractions (`dict` vs. `object`) into `_get_field`, `_build_scaffold_lines`, and `_build_feature_map_lines` helper functions to reduce the main `print_demo_report` function's cyclomatic complexity.
