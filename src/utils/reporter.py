...
def _create_top_border(title: str, width: int, color: str) -> str:
    ...
    return (f"{TL_CORNER}{H_LINE * left_pad}{Colors.BOLD}{title_text}"
...
def print_panel(
    ...
    for line in wrapped_lines:
        ...
        if padding < 0:
            padding = 0
        print(f"{color}{V_LINE}{Colors.ENDC} {line}{' ' * padding} "
...
def print_workflow_report(
    ...
    print(f"{Colors.BOLD}🔗 Repo:   {Colors.ENDC} "
    ...
    if session_id:
        ...
        print(f"{Colors.YELLOW}⚠️  Jules session was not created "
...
def print_session_status(
    ...
    status_msg = (f"{Colors.GREEN}✅ Yes{Colors.ENDC}" if is_complete
                  else f"{Colors.YELLOW}⏳ In Progress{Colors.ENDC}")
...
def print_watch_complete(elapsed: int, pr_url: Optional[str] = None) -> None:
    ...
    if pr_url:
        ...
        print(f"{Colors.YELLOW}ℹ️  Session completed but no PR was "
              f"created.{Colors.ENDC}")
