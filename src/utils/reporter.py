...
def _build_scaffold_tree(scaffold: dict[str, Any]) -> str:
    """Builds the scaffold file tree text."""
...
def _build_feature_list(items: list[Any], title: str, limit: int) -> list[str]:
    """Builds a formatted list of features."""
    if not items:
        return []
...
def _build_feature_maps_summary(feature_maps: dict[str, Any]) -> str:
    """Builds the feature maps summary text."""
    mvp = feature_maps.get("mvp_features", [])
    prod = feature_maps.get("production_features", [])
    fm_lines = []

    if mvp:
        fm_lines.extend(_build_feature_list(mvp, "🎯 MVP Features", 5))

    if prod:
        if fm_lines:
            fm_lines.append("")
        fm_lines.extend(_build_feature_list(prod, "🚀 Production Features", 3))
...
def print_demo_report(
    ...
    # --- Scaffold file tree ---
    tree_text = _build_scaffold_tree(scaffold)
    print_panel(tree_text, title="🏗️  MVP Scaffold Preview", color=Colors.BLUE, width=70)
    print("")

    # --- Feature maps summary ---
    if feature_maps:
        fm_text = _build_feature_maps_summary(feature_maps)
        if fm_text:
            print_panel(fm_text, title="📋 Feature Maps", color=Colors.HEADER, width=70)
            print("")
