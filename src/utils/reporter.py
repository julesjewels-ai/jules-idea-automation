...
def _extract_val(obj: Any, key: str) -> str:
    """Extracts a value from a dictionary or object."""
...
def _build_scaffold_tree(scaffold: dict[str, Any]) -> list[str]:
    """Builds the scaffold file tree lines."""
    files = scaffold.get("files", [])
    tree_lines = [f"{Colors.BOLD}📂 Generated Scaffold ({len(files)} files):{Colors.ENDC}"]
    for f in files:
        path = _extract_val(f, "path")
        desc = _extract_val(f, "description")
...
def _build_feature_lines(items: list[Any], title: str, limit: int) -> list[str]:
    """Builds feature list lines."""
    if not items:
        return []
    lines = [f"{Colors.BOLD}{title} ({len(items)} items):{Colors.ENDC}"]
    for item in items[:limit]:
        name = _extract_val(item, "name")
        prio = _extract_val(item, "priority")
...
def print_demo_report(
    ...
    tree_lines = _build_scaffold_tree(scaffold)
    print_panel("\n".join(tree_lines), title="🏗️  MVP Scaffold Preview", color=Colors.BLUE, width=70)
    print("")

    if feature_maps:
        fm_lines = []
        mvp_lines = _build_feature_lines(feature_maps.get("mvp_features", []), "🎯 MVP Features", 5)
        if mvp_lines:
            fm_lines.extend(mvp_lines)

        prod_lines = _build_feature_lines(feature_maps.get("production_features", []), "🚀 Production Features", 3)
        if prod_lines:
            if fm_lines:
                fm_lines.append("")
