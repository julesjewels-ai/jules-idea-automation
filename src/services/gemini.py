...
from src.services.cache import smart_cache
...
CATEGORY_PROMPTS = {
    "web_app": ("Generate a creative web application idea. Focus on modern "
                "frontend frameworks and responsive design."),
    "cli_tool": ("Generate a useful command-line tool idea. Focus on "
                 "developer productivity and Unix philosophy."),
    "api_service": ("Generate a RESTful API service idea. Focus on "
                    "microservices architecture and scalability."),
    "mobile_app": ("Generate a mobile application idea. Focus on user "
                   "experience and cross-platform compatibility."),
    "automation": ("Generate an automation tool idea. Focus on workflow "
                   "optimization and integration capabilities."),
    "ai_ml": ("Generate an AI/ML application idea. Focus on practical use "
              "cases and accessible interfaces."),
    "default": ("Generate a creative, unique, and useful software "
                "application idea.")
}
...
class GeminiClient:
        ...
        if not self.api_key:
            raise ConfigurationError(
                ...
                tip=("Get your API key from "
                     "https://aistudio.google.com/app/apikey and "
    ...
    def _generate_content_key_builder(
            self,
            prompt: str,
            schema: Any,
            error_tip: str) -> str:
        """Builds a cache key for _generate_content."""
        schema_name = getattr(schema, "__name__", str(schema))
        return f"gemini:{self.model_name}:{schema_name}:{prompt}"

    @smart_cache(key_builder=_generate_content_key_builder)
    ...
    def generate_idea(self, category: Optional[str] = None) -> dict[str, Any]:
        ...
        Args:
            category: Optional category to target (web_app, cli_tool, etc)
        """
        base_prompt = CATEGORY_PROMPTS.get(
            category or "default", CATEGORY_PROMPTS["default"])
        prompt = (f"{base_prompt} Include recommended tech stack and "
                  "key MVP features.")

        return self._generate_content(
            prompt,
            IdeaResponse,
            ("The AI model returned invalid JSON. Please try again or "
    ...
    def extract_idea_from_text(self, text: str) -> dict[str, Any]:
        ...
        described.
        Summarize it into a clear, actionable project description suitable
        for a developer to start building.
        ...
        return self._generate_content(
            prompt,
            IdeaResponse,
            ("The AI model returned invalid JSON while analyzing the "
             "website content.")
        )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.error(
                        f"Scaffold generation failed after {max_retries + 1} "
                        f"attempts: {e}"
                    )
