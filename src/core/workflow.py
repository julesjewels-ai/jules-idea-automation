...
class IdeaWorkflow:
    ...
    def _generate_scaffold(self, username: str, idea_data: dict, verbose: bool) -> None:
        ...
        files_to_create = self._prepare_files_for_commit(scaffold)
        ...
        if files_to_create:
            ...
            if verbose:
                print(
                    f"  Created {result['files_created']} files in one commit")

    def _prepare_files_for_commit(self, scaffold: dict) -> list[dict]:
        """Prepare list of files to commit from scaffold data."""
        files_to_create = []
