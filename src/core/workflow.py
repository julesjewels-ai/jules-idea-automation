...
class IdeaWorkflow:
    """Orchestrates creation of a GitHub repo and Jules session from an idea.
    ...
    def execute(
        ...
        Args:
            ...
            verbose: Deprecated. Use EventListener to capture output.
    ...
    def _create_repository(
        ...
        self.bus.publish(StepStarted(step_name=step_name))
        ...
        try:
            ...
            self.bus.publish(
                StepCompleted(step_name=step_name, result=username)
            )
            return username
        except Exception as e:
            self.bus.publish(StepFailed(step_name=step_name, error=str(e)))
            raise e
    ...
    def _generate_scaffold(
        ...
        step_name = "Generating MVP scaffold with Gemini"
        self.bus.publish(StepStarted(step_name=step_name))

        try:
            scaffold = self.gemini.generate_project_scaffold(idea_data)

            # Sub-step: Build README and Commit
            commit_step = "Committing README and scaffold files"
            self.bus.publish(StepStarted(step_name=commit_step))
            ...
            self.bus.publish(
                StepCompleted(
                    step_name=commit_step,
                    result=f"{len(files_to_create)} files added"
                )
            )

            # Complete the parent step
            self.bus.publish(StepCompleted(step_name=step_name))
    ...
    def _create_jules_session(
        ...
        step_name = f"Waiting for Jules to discover '{idea_data['slug']}'"
        ...
        try:
            ...
            if not source_found:
                err_msg = (
                    f"Source '{source_id}' was not found in Jules after "
                    f"{timeout}s. Please visit https://jules.google.com to "
                    "install the app."
                )
                self.bus.publish(
                    StepFailed(step_name=step_name, error=err_msg)
                )
                return None

            self.bus.publish(
                StepCompleted(step_name=step_name, result="Source found")
            )

            session_step = "Creating Jules session"
            self.bus.publish(StepStarted(step_name=session_step))
            ...
            self.bus.publish(
                StepCompleted(step_name=session_step, result=session.get('id'))
            )
            return session
