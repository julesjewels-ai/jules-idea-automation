...
class IdeaWorkflow:
        ...
        if scaffold.get("files"):
            ...
            for file_info in scaffold["files"]:
                if file_info["path"].lower() == "readme.md":
                    continue
                files_to_create.append({"path": file_info["path"], "content": file_info["content"]})
    ...
    def _create_jules_session(self, username: str, idea_data: dict, timeout: int, verbose: bool) -> Optional[dict]:
